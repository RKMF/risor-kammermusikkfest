...
export const compositeArtistPublishAction: DocumentActionComponent = (props) => {
  ...
  const handlePublish = useCallback(async () => {
    ...
    let orphaned: string[] = [];
    try {
      orphaned = await findOrphanedReciprocalReferences(client, id, 'events', 'event', 'artist');
    } catch (error) {
      ...
      const names = await Promise.all(
        missing.map(async (eventId) => {
          const event = await client.fetch<{
            ...
            title_no?: string;
            title_en?: string;
          }>(`*[_id == $id][0]{title, title_no, title_en}`, { id: eventId });
        ...
        ? {
            ...
            content: (
              <Stack space={4} padding={4}>
                ...
                <Text>
                  Vil du legge til denne artisten automatisk i{' '}
                  {eventCount === 1 ? 'arrangementet' : 'arrangementene'}?
                </Text>
                <Text size={1} muted>
          ...
          ? {
              ...
              content: (
                <Stack space={4} padding={4}>
                  ...
                  <Text>
                    Vil du fjerne denne artisten automatisk fra{' '}
                    {orphanedEventCount === 1 ? 'arrangementet' : 'arrangementene'}?
                  </Text>
                  <Text size={1} muted>
