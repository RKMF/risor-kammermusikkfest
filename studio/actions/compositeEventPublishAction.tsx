...
export const compositeEventPublishAction: DocumentActionComponent = (props) => {
  ...
  const handlePublish = useCallback(async () => {
    ...
    let missing: string[] = [];
    try {
      missing = await findMissingReciprocalReferences(client, id, 'artist', 'artist', 'events');
    } catch (error) {
    ...
    let orphaned: string[] = [];
    try {
      orphaned = await findOrphanedReciprocalReferences(client, id, 'artist', 'artist', 'events');
    } catch (error) {
    ...
    if (missing.length > 0) {
      ...
      const names = await Promise.all(
        missing.map(async (artistId) => {
          const artist = await client.fetch<{ name: string }>(`*[_id == $id][0]{name}`, {
            id: artistId,
          });
  ...
  const handleArtistSync = useCallback(
    async (shouldSync: boolean) => {
      ...
      try {
        await syncReciprocalReferences(client, id, missingArtistRefs, 'events');
        ...
        ? {
            ...
            content: (
              <Stack space={4} padding={4}>
                ...
                <Text>
                  Vil du legge til dette arrangementet automatisk hos{' '}
                  {artistCount === 1 ? 'artisten' : 'artistene'}?
                </Text>
                <Text size={1} muted>
        ...
        : orphanedArtistDialogOpen
          ? {
              ...
              content: (
                <Stack space={4} padding={4}>
                  ...
                  <Text>
                    Vil du fjerne dette arrangementet automatisk fra{' '}
                    {orphanedArtistCount === 1 ? 'artisten' : 'artistene'}?
                  </Text>
                  <Text size={1} muted>
          ...
          : programDialogOpen
            ? {
                ...
                content: (
                  <Stack space={4} padding={4}>
                    <Text>Vil du at dette arrangementet skal vises på programsiden?</Text>
                    <Text size={1} muted>
