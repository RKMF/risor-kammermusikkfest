...
export const generateEventScrollHtml: ComponentHTMLGenerator<EventScrollContainerData> = (
  ...
  const itemsHtml = sortedEvents
    .map((event) => {
      ...
      const dateHtml =
        data.showDate && dateString
          ? `<div class="event-date">${escapeHtml(dateString)}</div>`
          : '';
      const timeHtml =
        data.showTime && timeString
          ? `<div class="event-time">${escapeHtml(timeString)}</div>`
          : '';
      const venueHtml =
        data.showVenue && venueName
          ? `<div class="event-venue">${escapeHtml(venueName)}</div>`
          : '';
      const artistsHtml =
